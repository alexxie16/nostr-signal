...
import { hasNostrExtension, normalizeShopSlug, postNote } from "@/lib/postNostr";
...
export function PostForm() {
  ...
  const normalizedShopSlug = normalizeShopSlug(shopName);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!normalizedShopSlug) {
      setMessage({ type: "error", text: "Shop name must contain letters or numbers" });
      return;
    }
    if (!content.trim()) {
    ...
    setLoading(true);
    try {
      const result = await postNote(location, domain, shopName, content);
      if ("error" in result) {
        setMessage({ type: "error", text: result.error });
      } else {
        setMessage({
          type: "success",
          text: `Posted for #${result.shopSlug}! Event ID: ${result.eventId.slice(0, 16)}…`,
        });
        setShopName("");
        setContent("");
      }
        ...
        >
          ...
          <div className="mt-4">
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {normalizedShopSlug
                ? <>This will be tagged as <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-gray-700">{normalizedShopSlug}</code> on Nostr.</>
                : "We’ll turn the name into a lowercase slug tag automatically."}
            </p>
          </div>
          <div className="mt-4">
            <label
